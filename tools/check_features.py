...
def main():
    parser = argparse.ArgumentParser(
        description="Per-class feature statistics and drift detector"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mlflow",
        action="store_true",
        help="Log feature stats and drift metrics to MLflow",
    )
    parser.add_argument(
        "--mlflow-uri", default="http://10.10.130.10:5000", help="MLflow server URI"
    )
    parser.add_argument(
        "--run-id", default=None, help="MLflow active run ID to log directly inside"
    )
    parser.add_argument(
        "--round", type=int, default=0, help="Current training round (global step)"
    )
    parser.add_argument(
        "--tensorboard",
        action="store_true",
        help="Write feature histograms to TensorBoard",
    )
    parser.add_argument(
        "--tb-log-dir", default="runs/feature_drift", help="TensorBoard log directory"
    )
    ...
    if args.baseline_json:
        ...
        if baseline_path.exists():
            ...
            sys.stderr.write(
                f"Baseline JSON path {args.baseline_json} does not exist. Skipping Z-score comparisons.\n"
            )
    ...
    for label in classes:
        ...
        for col in available_features:
            ...
            current_stats[str(label)][col] = {"mean": mean_val, "std": std_val}
            ...
            print(
                f"  {col:>25s}:  mean={mean_val:>10.2f}  std={std_val:>10.2f}  "
        ...
        try:
            ...
            def log_to_run():
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    mlflow.log_artifact(
                        str(stat_file), artifact_path="feature_drift_reports"
                    )
                ...
                if not sub.empty:
                    for col in available_features:
                        writer.add_histogram(
                            f"Features_Class_{label}/{col}",
                            sub[col].values,
                            global_step=args.round,
                        )
            writer.close()
