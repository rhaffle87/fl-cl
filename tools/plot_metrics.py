...
def run_plotting(
    key_path, aggregator_ip, local_db="mlflow_temp.db", run_id_arg=None, output_dir=None
):
    """
    ...
    if output_dir is None:
        ...
        output_dir = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "data", "exports", "plots")
        )
    ...
    backup_cmd = (
        ["ssh", "-n"]
        + ssh_opts
        + [f"root@{aggregator_ip}", f'python3 -c "{backup_script}"']
    )
    ...
    if key_path:
        scp_opts += ["-i", key_path]
    scp_cmd = (
        ["scp"] + scp_opts + [f"root@{aggregator_ip}:/tmp/mlflow_backup.db", local_db]
    )
    ...
    cleanup_remote_cmd = (
        ["ssh", "-n"]
        + ssh_opts
        + [f"root@{aggregator_ip}", "rm -f /tmp/mlflow_backup.db"]
    )
    ...
    cur.execute(
        "SELECT key, value, step FROM metrics WHERE run_uuid = ? ORDER BY step ASC, key ASC",
        (run_id,),
    )
    ...
    for col, (display_name, file_suffix, color) in classes.items():
        ...
        ax1.set_title(
            f"FL-CL Metrics: {display_name} (Run: {run_id[:8]})",
            fontsize=14,
            fontweight="bold",
            pad=15,
        )
        ...
        global_acc = (
            ...
            else df.get("val_accuracy", df.get("acc", []))
        )
        if len(global_acc) > 0:
            ax2.plot(
                df["Round"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Export and plot MLflow metrics for FL-CL experiments."
    )
    ...
    parser.add_argument(
        "--db", default="mlflow_temp.db", help="Local temporary DB name"
    )
    parser.add_argument(
        "--run-id", default=None, help="Specific Run ID to plot (defaults to latest)"
    )
    parser.add_argument(
        "--output-dir", default=None, help="Directory to save the generated plots"
    )
