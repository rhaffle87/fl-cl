...
avalanche.benchmarks.scenarios.dataset_scenario = types.ModuleType("avalanche.benchmarks.scenarios.dataset_scenario")
...
avalanche.benchmarks.utils.make_tensor_classification_dataset = mock_make_tensor_classification_dataset
avalanche.benchmarks.utils.as_classification_dataset = mock_as_classification_dataset
avalanche.benchmarks.scenarios.dataset_scenario.benchmark_from_datasets = mock_benchmark_from_datasets
...
sys.modules["avalanche.benchmarks.scenarios.dataset_scenario"] = avalanche.benchmarks.scenarios.dataset_scenario
    ...
    if skewed:
        ...
        for _ in range(50):
            data.append({
                ...
                "bidirectional_packets": 100.0, "bidirectional_bytes": 10000.0,
                "src2dst_packets": 50.0, "src2dst_bytes": 5000.0,
                "dst2src_packets": 50.0, "dst2src_bytes": 5000.0,
                "src2dst_mean_piat_ms": 2.0, "dst2src_mean_piat_ms": 2.0
            })
    else:
        ...
        for _ in range(10):
            data.append({
                ...
                "duration_ms": 100.0,
                "bidirectional_packets": 10.0, "bidirectional_bytes": 1000.0,
                "src2dst_packets": 5.0, "src2dst_bytes": 500.0,
                "dst2src_packets": 5.0, "dst2src_bytes": 500.0,
                "src2dst_mean_piat_ms": 20.0, "dst2src_mean_piat_ms": 20.0
            })
        # Class 1: Botnet (C2: port 8080)
        for _ in range(10):
            data.append({
                "src_ip": "10.10.140.10", "dst_ip": "192.168.1.100",
                "src_port": 1234, "dst_port": 8080,
                "duration_ms": 500.0,
                "bidirectional_packets": 30.0, "bidirectional_bytes": 3000.0,
                "src2dst_packets": 15.0, "src2dst_bytes": 1500.0,
                "dst2src_packets": 15.0, "dst2src_bytes": 1500.0,
                "src2dst_mean_piat_ms": 10.0, "dst2src_mean_piat_ms": 10.0
            })
        ...
        for _ in range(10):
            data.append({
                ...
                "duration_ms": 50.0,
                "bidirectional_packets": 4.0, "bidirectional_bytes": 400.0,
                "src2dst_packets": 2.0, "src2dst_bytes": 200.0,
                "dst2src_packets": 2.0, "dst2src_bytes": 200.0,
                "src2dst_mean_piat_ms": 5.0, "dst2src_mean_piat_ms": 5.0
            })
        ...
        for _ in range(10):
            data.append({
                ...
                "duration_ms": 1000.0,
                "bidirectional_packets": 20.0, "bidirectional_bytes": 2000.0,
                "src2dst_packets": 10.0, "src2dst_bytes": 1000.0,
                "dst2src_packets": 10.0, "dst2src_bytes": 1000.0,
                "src2dst_mean_piat_ms": 50.0, "dst2src_mean_piat_ms": 50.0
            })
        ...
        for _ in range(10):
            data.append({
                ...
                "src_port": 1234, "dst_port": 80,
                "duration_ms": 3000.0,
                "bidirectional_packets": 50.0, "bidirectional_bytes": 5000.0,
                "src2dst_packets": 25.0, "src2dst_bytes": 2500.0,
                "dst2src_packets": 25.0, "dst2src_bytes": 2500.0,
                "src2dst_mean_piat_ms": 5.0, "dst2src_mean_piat_ms": 5.0
            })
...
def test_architectures():
    ...
    for m_type in ["mlp", "cnn", "transformer"]:
        ...
        try:
            ...
            model.eval()
            out = model(x)
            assert out.shape == (batch_size, num_classes), f"Output shape mismatch: {out.shape}"
            ...
            try:
                ...
                if m_type == "transformer":
                    results[f"{m_type}_quantization"] = "SKIP (Transformer library limitation)"
                else:
            ...
            try:
                ...
                for name, param in pruned_model.named_parameters():
                    if "weight" in name:
                        dummy_fisher[name] = np.random.uniform(0.1, 1.0, size=param.shape)
                ...
                if m_type == "transformer":
                    results[f"{m_type}_pruning"] = "SKIP (Transformer library limitation)"
                else:
...
def test_labeling_and_data_quality():
    ...
    labels = client.assign_labels_vectorized(df, dos_threshold_ms=2000, traffic_gen_ip="10.10.140.10")
    ...
    device = torch.device("cpu")
    cl_strat = get_continual_learner(model, device, "EWC") # Using mock EWC to check fisher diagonals extraction as well
    
    # Client with matching baseline -> should PASS gate
    client_pass = client.CyberDefenseClient(
        net=model, cl_strategy=cl_strat, flows_dir=TEST_DIR, client_id="ClientPass",
        baseline="10,10,10,10,10", js_threshold=0.6, traffic_gen_ip="10.10.140.10"
    )
    _, num_s, metrics = client_pass.fit(client_pass.get_parameters(config={}), {"server_round": 1})
    print(f"  ClientPass: JSD={metrics['dataset_jsd']:.4f}, rejected={metrics['dataset_rejected']}, samples={num_s}")
    assert metrics["dataset_rejected"] == 0.0, "Balanced dataset incorrectly rejected!"
    print(f"  ClientPass: Fisher Mean={metrics['fisher_mean']:.6f}, Fisher Max={metrics['fisher_max']:.6f}")
    ...
    client_fail = client.CyberDefenseClient(
        net=model, cl_strategy=cl_strat, flows_dir=TEST_DIR, client_id="ClientFail",
        baseline="10,10,10,10,10", js_threshold=0.6, traffic_gen_ip="10.10.140.10"
    )
    _, num_s, metrics_fail = client_fail.fit(client_fail.get_parameters(config={}), {"server_round": 1})
    print(f"  ClientFail: JSD={metrics_fail['dataset_jsd']:.4f}, rejected={metrics_fail['dataset_rejected']}, samples={num_s}")
    assert metrics_fail["dataset_rejected"] == 1.0, "Skewed dataset incorrectly accepted by the gate!"
...
def test_differential_privacy():
    ...
    cl_nodp = get_continual_learner(model=model_nodp, device=device, strategy_name="Naive", lr=0.1, dp_enabled=False)
...
def test_label_poisoning():
    ...
    poison_client = client.CyberDefenseClient(
        net=model, cl_strategy=cl_strat, flows_dir=TEST_DIR, client_id="PoisonClient",
        poison_enabled=True, poison_rate=1.0, poison_from=0, poison_to=4,
        js_threshold=1.0 # disable gate check by setting threshold high
    )
    
    # Get original labels from mock folder
    X_orig, y_orig = client.load_ramdisk_flows(TEST_DIR, dos_threshold_ms=2000, traffic_gen_ip="10.10.140.10")
    ...
    _, _, fit_metrics = poison_client.fit(poison_client.get_parameters(config={}), {"server_round": 1})
    ...
    print(f"  After Poison Simulation: Class 0 count: {poisoned_zeros}, Class 4 count: {poisoned_fours}")
...
def test_server_aggregation_and_drift():
    ...
    g_weights = [np.array([[1.0, 2.0], [3.0, 4.0]], dtype=np.float32), np.array([0.5, 0.5], dtype=np.float32)]
    
    # Client 1: Close to global
    c1_weights = [np.array([[1.1, 1.9], [3.1, 4.1]], dtype=np.float32), np.array([0.6, 0.4], dtype=np.float32)]
    # Client 2: Divergent
    c2_weights = [np.array([[2.0, 3.0], [4.0, 5.0]], dtype=np.float32), np.array([1.5, 1.5], dtype=np.float32)]
    # Client 3: Malicious containing NaN
    c3_weights = [np.array([[np.nan, 2.0], [3.0, np.inf]], dtype=np.float32), np.array([0.5, 0.5], dtype=np.float32)]
    ...
    expected_drift_c1 = np.sqrt(0.1**2 + (-0.1)**2 + 0.1**2 + 0.1**2 + 0.1**2 + (-0.1)**2)
    ...
    assert np.allclose(ndarrays_median[0], np.array([[1.1, 2.0], [3.1, 4.1]], dtype=np.float32)), "FedMedian mismatch!"
    ...
    print("  Aggregated Weights (TrimmedMean beta=0.33) layer 0:\n", ndarrays_trimmed[0])
    assert np.allclose(ndarrays_trimmed[0], np.array([[1.1, 2.0], [3.1, 4.1]], dtype=np.float32)), "TrimmedMean mismatch!"
