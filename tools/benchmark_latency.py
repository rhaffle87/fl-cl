...
# Resolve local paths
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "src", "defender")))
from model import get_model

OUTPUT_CSV = os.path.join(os.path.dirname(__file__), "..", "data", "reports", "latency_quantization_report.csv")
    ...
    for arch in architectures:
        ...
        try:
            ...
            try:
                ...
                int8_latency, int8_throughput = measure_latency(quantized_model, dummy_input)
            ...
            print(f"    FP32:  {fp32_latency:6.3f} ms/batch | {fp32_throughput:8.1f} flows/sec | Size: {param_size:.3f} MB")
            print(f"    INT8:  {int8_latency:6.3f} ms/batch | {int8_throughput:8.1f} flows/sec | Size: {int8_param_size:.3f} MB (Speedup: {speedup:.2f}x)")
