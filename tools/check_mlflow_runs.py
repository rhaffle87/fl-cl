...
def inspect_all_runs():
    ...
    if not os.path.exists(db_path):
        ...
        if os.path.exists("data/db/matrix_mlflow.db"):
            db_path = "data/db/matrix_mlflow.db"
        else:
    ...
    pivoted_m = latest_metrics.pivot(
    ...
    df_full["duration_s"] = (
        (df_full["end_time"] - df_full["start_time"]) / 1000.0
    ).round(1)
    df_full["start_dt"] = pd.to_datetime(df_full["start_time"], unit="ms").dt.strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        for _, exp in df_exp.iterrows():
            ...
            f.write(
                f"| `{exp['experiment_id']}` | **{exp['name']}** | `{count}` | `{exp['artifact_location']}` |\n"
            )
        f.write("\n---\n\n")
        ...
        for exp_id in sorted(df_full["experiment_id"].unique()):
            ...
            for _, row in sub_df.iterrows():
                ...
                acc = (
                    ...
                    if pd.notnull(row.get("accuracy"))
                    else "N/A"
                )
                loss = f"{row['loss']:.4f}" if pd.notnull(row.get("loss")) else "N/A"
                f1 = (
                    f"{row['crucial_model_performance']*100:.2f}%"
                    if pd.notnull(row.get("crucial_model_performance"))
                    else "N/A"
                )
                dur = (
                    f"{row['duration_s']:.0f}s"
                    if pd.notnull(row.get("duration_s"))
                    else "N/A"
                )

                c0 = (
                    f"{row['accuracy_class_0']*100:.1f}%"
                    if pd.notnull(row.get("accuracy_class_0"))
                    else "—"
                )
                c1 = (
                    f"{row['accuracy_class_1']*100:.1f}%"
                    if pd.notnull(row.get("accuracy_class_1"))
                    else "—"
                )
                c2 = (
                    f"{row['accuracy_class_2']*100:.1f}%"
                    if pd.notnull(row.get("accuracy_class_2"))
                    else "—"
                )
                c3 = (
                    f"{row['accuracy_class_3']*100:.1f}%"
                    if pd.notnull(row.get("accuracy_class_3"))
                    else "—"
                )
                c4 = (
                    f"{row['accuracy_class_4']*100:.1f}%"
                    if pd.notnull(row.get("accuracy_class_4"))
                    else "—"
                )

                f.write(
                    f"| {run_short} | `{model}` | `{cl}` | `{agg}` | `{dp}` | **{acc}** | {loss} | {f1} | {dur} | {c0} | {c1} | {c2} | {c3} | {c4} |\n"
                )
            f.write("\n")

    print(
        f"[SUCCESS] Exported detailed inspection of {len(df_full)} runs to {report_file}"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="MLflow Active and Completed Run Diagnostics Inspector"
    )
    _ = parser.parse_args()
    inspect_all_runs()
