...
def generate_mock_quarantine_flows(num_samples: int = 100, input_dim: int = 32):
    ...
    X = np.random.normal(loc=2.5, scale=1.2, size=(num_samples, input_dim)).astype(
        np.float32
    )
...
def load_quarantined_flows(quarantine_path: Path, input_dim: int = 32):
    ...
    if not quarantine_path.exists() or quarantine_path.stat().st_size == 0:
        print(
            f"[*] Quarantine buffer empty or not found at {quarantine_path}. Generating synthetic zero-day dataset..."
        )
    ...
    if not features:
        print(
            "[*] No valid feature vectors parsed from quarantine JSONL. Using synthetic fallback."
        )
        return generate_mock_quarantine_flows(num_samples=100, input_dim=input_dim)
...
def train_continual_quarantine(
    ...
    output_report: str = "data/reports/benchmarks/quarantine_retrain_report.csv",
    epochs: int = 5,
    lr: float = 0.005,
    dry_run: bool = False,
):
    ...
    print(
        "[*] Initializing episodic memory buffer (Classes 0-3 historical baseline)..."
    )
    np.random.seed(1337)
    ...
    with torch.no_grad():
        ...
        acc_hist_pre = (
            (out_hist.argmax(dim=1).cpu() == torch.tensor(y_hist)).float().mean().item()
        )
    print(
        f"[*] Historical Baseline Pre-Adaptation Accuracy: {acc_hist_pre * 100:.2f}%\n"
    )
    ...
    print(
        f"[*] Executing A-GEM continual retraining for {epochs} epochs on quarantined zero-days..."
    )
    model.train()
    ...
    with torch.no_grad():
        ...
        acc_hist_post = (
            (out_hist_post.argmax(dim=1).cpu() == torch.tensor(y_hist))
            .float()
            .mean()
            .item()
        )
    ...
    alerts.send_promotion_alert(
        f"CyberDefense{model_type.upper()}-ZeroDay-Adapter", "2.1", metrics
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="FL-CL Zero-Day Quarantine Continual Retraining Loop"
    )
    parser.add_argument(
        "--model-type",
        choices=["cnn", "mlp", "transformer"],
        default="cnn",
        help="Champion backbone architecture",
    )
    parser.add_argument(
        "--quarantine-file",
        default="data/quarantine/zero_day_flows.jsonl",
        help="Path to quarantined zero-day JSONL flows",
    )
    parser.add_argument(
        "--output",
        default="data/reports/benchmarks/quarantine_retrain_report.csv",
        help="Path to save CSV scorecard",
    )
    parser.add_argument(
        "--epochs", type=int, default=5, help="Number of continual adaptation epochs"
    )
    parser.add_argument(
        "--lr", type=float, default=0.005, help="Learning rate for A-GEM optimizer"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate pipeline without updating model weights",
    )
    args = parser.parse_args()

    sys.exit(
        train_continual_quarantine(
