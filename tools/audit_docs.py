...
for md_file in md_files:
    ...
    for match in link_pattern.finditer(content):
        ...
        if (link_path.startswith("http://") or link_path.startswith("https://") or 
            link_path.startswith("mailto:") or link_path.startswith("#")):
            continue
        ...
        if resolved and not resolved.exists():
            ...
            if is_ci and any(f"/{d}/" in rel_resolved for d in ["data", "scratch", "runs", "exports"]):
                warnings.append(f"[{md_file.relative_to(repo_root)}] Runtime/Ignored Target Missing in CI: [{text}]({link_path})")
            else:
