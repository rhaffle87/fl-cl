...
def run_dlg_attack(model, clean_x, clean_y, dp_noise_sigma=0.0, max_iter=300):
    ...
    for it in range(max_iter):
        ...
        dummy_loss = nn.functional.cross_entropy(
            dummy_pred, nn.functional.softmax(dummy_label, dim=-1)
        )
...
def run_sybil_byzantine_benchmark(num_clients=5, sybil_fraction=0.40, rounds=5):
    ...
    for strat in strategies:
        ...
        for r in range(rounds):
            ...
            if strat == "FedAvg":
                agg_w = [
                    np.mean([w[l] for w in local_weights], axis=0)
                    for l in range(len(local_weights[0]))
                ]
            elif strat == "FedMedian":
                agg_w = [
                    np.median([w[l] for w in local_weights], axis=0)
                    for l in range(len(local_weights[0]))
                ]
                ...
                for l in range(len(local_weights[0])):
                    stacked = np.sort(
                        np.stack([w[l] for w in local_weights], axis=0), axis=0
                    )
                    trimmed = (
                        stacked[k:-k] if (k > 0 and 2 * k < num_clients) else stacked
                    )
                    agg_w.append(np.mean(trimmed, axis=0))
            elif strat == "Krum":
                flat = [
                    np.concatenate([arr.flatten() for arr in w]) for w in local_weights
                ]
                ...
                scores = [
                    np.sum(np.sort(dists[i])[1 : num_clients - num_byzantine - 1])
                    for i in range(num_clients)
                ]
...
def main():
    ...
    byz_results = run_sybil_byzantine_benchmark(
        num_clients=5, sybil_fraction=0.40, rounds=5
    )
    for strat, acc in byz_results.items():
        print(
            f"  - Aggregator [{strat:<12s}]: Accuracy under 40% Sybil Attack = {acc*100:.2f}%"
        )
    ...
    mse_clean = run_dlg_attack(
        model, sample_x, sample_y, dp_noise_sigma=0.0, max_iter=200
    )
    mse_dp = run_dlg_attack(
        model, sample_x, sample_y, dp_noise_sigma=0.30, max_iter=200
    )
    print(
        f"  - Non-DP (sigma=0.00) Feature Inversion Reconstruction MSE:  {mse_clean:.4f} (Vulnerable)"
    )
    print(
        f"  - DP-SGD (sigma=0.30) Feature Inversion Reconstruction MSE:  {mse_dp:.4f} (Protected / Randomized)"
    )
    ...
    report_df = pd.DataFrame(
        [
            {
                "Test": "Byzantine 40% Sybil (FedAvg)",
                "Metric": "Accuracy",
                "Value": byz_results.get("FedAvg"),
            },
            {
                "Test": "Byzantine 40% Sybil (TrimmedMean)",
                "Metric": "Accuracy",
                "Value": byz_results.get("TrimmedMean"),
            },
            {
                "Test": "Byzantine 40% Sybil (FedMedian)",
                "Metric": "Accuracy",
                "Value": byz_results.get("FedMedian"),
            },
            {
                "Test": "Byzantine 40% Sybil (Krum)",
                "Metric": "Accuracy",
                "Value": byz_results.get("Krum"),
            },
            {
                "Test": "DLG Gradient Inversion (Non-DP)",
                "Metric": "Reconstruction_MSE",
                "Value": mse_clean,
            },
            {
                "Test": "DLG Gradient Inversion (DP-SGD σ=0.30)",
                "Metric": "Reconstruction_MSE",
                "Value": mse_dp,
            },
        ]
    ...
    report_df.to_csv(
        "data/reports/benchmarks/adversarial_stress_benchmark.csv", index=False
    )
    print(
        "\n[SUCCESS] Saved adversarial benchmark report to data/reports/benchmarks/adversarial_stress_benchmark.csv"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Adversarial Flow Perturbation and Stress Benchmark"
    )
    _ = parser.parse_args()
    main()
