...
def plot_class_distribution(aggregate_labels):
    ...
    bars = ax.barh(df_labels["Class"], df_labels["Count"], color=bar_colors, edgecolor="#1A202C", linewidth=0.8, alpha=0.9)
    ax.set_xscale("log")
    ...
    for bar, count, pct in zip(bars, df_labels["Count"], df_labels["Percentage"]):
        ...
        ax.text(width * 1.25, bar.get_y() + bar.get_height() / 2,
                f"{count:,} flows ({pct_text})",
                va="center", ha="left", fontsize=8.5, fontweight="bold", color="#2D3748")

    ax.set_title("CIC-IDS2017 Dataset: Full Traffic Distribution & Class Imbalance (N = 2,830,743 Flows)", pad=15)
    ...
    ax.legend(handles=legend_elements, loc="lower right", frameon=True, facecolor="#F7FAFC", edgecolor="#CBD5E0", fontsize=8.5)
...
def plot_temporal_day_breakdown(day_class_matrix):
    ...
    print("[*] Generating Plot 2: Day-by-Day Temporal Continual Learning Task Structure...")
    ...
    p1 = ax.bar(x, benign_counts, width, label="Normal / BENIGN Traffic", color="#2B6CB0", edgecolor="#1A202C", alpha=0.9)
    p2 = ax.bar(x, attack_counts, width, bottom=benign_counts, label="Malicious Attack Traffic", color="#E53E3E", edgecolor="#1A202C", alpha=0.9)

    ax.set_title("CIC-IDS2017 Chronological Task Progression: Foundation for Continual Learning (CL) Tasks", pad=15)
    ...
    for i in range(len(day_titles)):
        ...
        ax.text(i, tot + 15000, f"{tot:,}\n({atk_pct:.1f}% Atk)", ha="center", va="bottom", fontsize=8, fontweight="bold", color="#2D3748")
...
def plot_attack_spectrum(aggregate_labels):
    ...
    colors = ["#E53E3E", "#C53030", "#ED8936", "#805AD5", "#319795", "#D69E2E", "#718096"]
    ...
    ax.set_title(f"CIC-IDS2017 Malicious Threat Spectrum (Total Attack Flows: {total_attacks:,})", pad=15)
    
    legend_labels = [f"{l}: {s:,} ({s/total_attacks*100:.2f}%)" for l, s in zip(labels, sizes)]
    ax.legend(wedges, legend_labels, title="Attack Threat Categories", loc="center left", bbox_to_anchor=(0.95, 0.5),
              fontsize=8.5, title_fontsize=9.5, frameon=True, facecolor="#F7FAFC", edgecolor="#CBD5E0")
...
def plot_benchmark_comparison_radar():
    ...
    print("[*] Generating Plot 4: Benchmark Comparison (Literature Baseline vs FL-CL Live Testbed)...")
    ...
    ax.plot(angles, values_baseline, color="#E53E3E", linewidth=2.2, linestyle="--", label="CIC-IDS2017 Literature (FedAvg / FL-IIDS)")
    ax.fill(angles, values_baseline, color="#E53E3E", alpha=0.15)

    ax.plot(angles, values_flcl, color="#38A169", linewidth=2.8, label="Our FL-CL System (GEM + TrimmedMean + 8-bit Quant)")
    ...
    plt.yticks([20, 40, 60, 80, 100], ["20%", "40%", "60%", "80%", "100%"], color="#718096", size=8.5)
    plt.ylim(0, 115)

    ax.set_title("Performance & Architectural Radar:\nCIC-IDS2017 Baseline vs. FL-CL Live Hypervisor Implementation", pad=35, fontsize=12.5)
    ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.08), ncol=1, frameon=True, facecolor="#F7FAFC", edgecolor="#CBD5E0", fontsize=9.5)
...
def plot_feature_comparison():
    ...
    if os.path.exists(f_bot):
        ...
        benign_rows = df_bot[df_bot[label_col].str.contains("BENIGN", na=False)].head(2000)
    ...
    if os.path.exists(f_dos):
        ...
        dos_rows = df_dos[df_dos[label_col].str.contains("slowloris", case=False, na=False)].head(2000)
    ...
    if os.path.exists(f_ssh):
        ...
        ssh_rows = df_ssh[df_ssh[label_col].str.contains("SSH-Patator", case=False, na=False)].head(2000)
    ...
    df_sample["DisplayClass"] = df_sample["CleanLabel"].map(label_map).fillna(df_sample["CleanLabel"])

    # Extract flow duration and packet length columns
    dur_col = [c for c in df_sample.columns if "flow duration" in c.lower() or "duration" in c.lower()][0]
    pkt_col = [c for c in df_sample.columns if "packet length mean" in c.lower() or "mean" in c.lower()][0]

    df_sample[dur_col] = pd.to_numeric(df_sample[dur_col], errors="coerce").fillna(0) + 1.0
    df_sample[pkt_col] = pd.to_numeric(df_sample[pkt_col], errors="coerce").fillna(0) + 1.0
    ...
    sns.boxplot(data=df_sample, x="DisplayClass", y=dur_col, hue="DisplayClass", palette=palette, ax=axes[0], order=order, showfliers=False, legend=False)
    ...
    sns.boxplot(data=df_sample, x="DisplayClass", y=pkt_col, hue="DisplayClass", palette=palette, ax=axes[1], order=order, showfliers=False, legend=False)
    ...
    fig.suptitle("Key Metadata Feature Signatures across CIC-IDS2017 Threat Classes", fontsize=13, fontweight="bold", y=1.02)
