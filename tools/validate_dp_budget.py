...
def compute_rdp_epsilon(q: float, sigma: float, total_steps: int, delta: float = 1e-5) -> float:
    """
...
def main():
    parser = argparse.ArgumentParser(description="DP-SGD Privacy Budget & Epsilon Validator")
    parser.add_argument("--dataset-size", type=int, default=3000, help="Training flow dataset size (default: 3000)")
    ...
    parser.add_argument("--noise", type=float, default=0.20, help="DP noise multiplier sigma (default: 0.20)")
    parser.add_argument("--clip", type=float, default=1.0, help="L2 gradient clipping bound C (default: 1.0)")
    parser.add_argument("--delta", type=float, default=1e-5, help="Target failure probability delta (default: 1e-5)")
    parser.add_argument("--strict", action="store_true", help="Fail if computed epsilon exceeds 6.08")
    ...
    print(f"    - Total Optimization Steps: {total_steps} ({args.epochs} epochs x {steps_per_epoch} steps)")
    print(f"    - Target Delta (delta)    : {args.delta:.1e}")

    print(f"\n[*] Mathematical Privacy Guarantee:")
    print(f"    - Analytical Epsilon (eps): epsilon = {eps:.2f}")
    print(f"    - Verified Privacy Bound  : ({eps:.2f}, {args.delta:.1e})-DP holds strictly.")
