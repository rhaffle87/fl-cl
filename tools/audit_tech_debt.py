...
def export_ledger(findings, export_path: Path):
    ...
    with open(export_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["file", "line", "marker", "severity", "snippet", "description"],
        )
        writer.writeheader()
        for row in findings:
            writer.writerow(row)
...
def main():
    parser = argparse.ArgumentParser(
        description="Technical Debt & Ponytail Ledger Auditor"
    )
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Exit with non-zero code if HIGH severity debt is found",
    )
    parser.add_argument(
        "--export",
        default=str(PROJECT_ROOT / "data" / "reports" / "technical_debt_ledger.csv"),
    ...
    if findings:
        ...
        print(
            "  [SUCCESS] Zero technical debt or unaddressed stubs found across scanned codebase!"
        )
    ...
    if args.strict and high_count > 0:
        print(
            f"[FAIL] Strict audit failed: {high_count} HIGH severity technical debt items require resolution."
        )
        sys.exit(1)
    else:
