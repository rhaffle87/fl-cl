...
def create_mock_flows():
    ...
    print("[OK] Created mock flows successfully!")
...
def run_test_training():
    ...
    try:
        ...
        X, y = client.load_ramdisk_flows(
            MOCK_FLOWS_DIR, dos_threshold_ms=2000, traffic_gen_ip="10.10.140.10"
        )
    ...
    for m_type in ["mlp", "cnn", "transformer"]:
        ...
        for epoch in range(3):
            ...
            print(
                f"  Epoch {epoch+1}/3 - Loss: {total_loss/total:.4f} | Accuracy: {correct/total:.4f}"
            )
        ...
        print(
            f"[OK] Finished training {m_type.upper()}. Final Train Accuracy: {acc:.4f}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Offline Synthetic Training Convergence Test Suite"
    )
    _ = parser.parse_args()
    run_test_training()
