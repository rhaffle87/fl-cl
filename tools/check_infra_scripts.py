...
def main():
    parser = argparse.ArgumentParser(
        description="Infrastructure Script & Line Ending Auditor"
    )
    parser.add_argument(
        "--fix",
        action="store_true",
        help="Automatically normalize CRLF to LF line endings",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Fail if any CRLF line endings or missing shebangs exist",
    )
    ...
    for r in results:
        le_str = (
            "LF" if not r["has_crlf"] else ("FIXED (LF)" if r["fixed"] else "CRLF [!]")
        )
    ...
    if args.strict and (crlf_count > 0 or missing_shebangs > 0):
        print(
            f"[FAIL] Infrastructure scripts audit failed: {crlf_count} CRLF, {missing_shebangs} missing shebangs."
        )
        sys.exit(1)
    else:
