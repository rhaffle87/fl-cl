...
Usage:
    ...
    # 2. Sync all benchmark CSVs in data/reports/ into separate sheet tabs
...
def get_default_url(cli_url: str = None) -> str:
    ...
    if "GOOGLE_SHEETS_WEBHOOK_URL" in os.environ and os.environ["GOOGLE_SHEETS_WEBHOOK_URL"]:
    ...
    if env_file.exists():
        with open(env_file, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line.startswith("GSHEETS_WEBHOOK_URL=") or line.startswith("GOOGLE_SHEETS_WEBHOOK_URL="):
...
def sync_all_reports(url: str, reports_dir: str = None) -> None:
    """Sync all CSV benchmark files in data/reports/."""
    if not reports_dir:
    ...
    for file_path in csv_files:
        ...
        tab_name = filename.replace("_benchmark", "").replace("_report", "").replace(".csv", "").replace("_", " ").title()[:30]
        if sync_csv_file(url, file_path, tab_name):
            success_count += 1

    print("=" * 65)
    print(f"[SUCCESS] Synced {success_count}/{len(csv_files)} benchmark reports to Google Sheets.")
    print("=" * 65)
...
def run_test(url: str) -> None:
    ...
    payload_promo = {
        ...
        "data": {
            ...
            "passed": True,
            "f1_scores": {"Normal": 0.9995, "Botnet": 1.00, "DNS Exfil": 0.998, "SSH Brute": 1.00, "DoS": 0.970},
...
def main():
    ...
    parser.add_argument("--url", default="", help="Google Apps Script WebApp URL (or set GSHEETS_WEBHOOK_URL env)")
    parser.add_argument("--sync-all", action="store_true", help="Sync all CSV benchmark reports in data/reports/")
    parser.add_argument("--file", default="", help="Path to a single CSV file to sync")
    parser.add_argument("--sheet", default="", help="Custom sheet tab name for the file")
    parser.add_argument("--test", action="store_true", help="Send test round and promotion events")
    args = parser.parse_args()

    webhook_url = get_default_url(args.url)
    if not webhook_url:
        print("[!] Error: No Google Sheets webhook URL provided.")
        print("    Pass --url 'https://script.google.com/macros/s/.../exec' or set GSHEETS_WEBHOOK_URL.")
        sys.exit(1)
