...
def main():
    parser = argparse.ArgumentParser(
        description="Inspect ramdisk flow label distribution"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Output JSON report with counts and JSD status",
    )
    parser.add_argument(
        "--dos-threshold-ms",
        type=float,
    ...
    parser.add_argument(
        "--baseline",
        default=None,
        help="Comma-separated relative baseline counts (e.g. 2,150,3,7,18)",
    )
    parser.add_argument(
        "--js-threshold",
        type=float,
    ...
    df["label"] = client.assign_labels_vectorized(
        df, dos_threshold_ms=args.dos_threshold_ms
    )
    ...
    if args.json:
        ...
        out_dict = {"counts": counts_full, "js_divergence": jsd_val, "status": status}
    ...
    if jsd_val is not None:
        print(
            f"Jensen-Shannon Divergence: {jsd_val:.4f} (Threshold: {args.js_threshold})"
        )
    ...
    for label in range(5):
        ...
        if not class_df.empty:
            ...
            print(
                class_df[["src_ip", "dst_ip", "src_port", "dst_port", "protocol"]]
                .head(5)
