...
def process_flow_file(file_path, model, device, threshold, alerts_log, stats_path=None):
    ...
    for idx, (pred_cls, conf) in enumerate(zip(predictions, confidences)):
        if pred_cls > 0 and conf >= threshold:
            ...
            _log.info(
                f"   Type:       {alert['predicted_label']} (Confidence: {alert['confidence']:.2%})"
            )
            _log.info(
                f"   Connection: {alert['src_ip']}:{alert['src_port']} -> {alert['dst_ip']}:{alert['dst_port']} (Proto: {alert['protocol']})"
            )
    ...
    if alerts_triggered > 0:
        _log.info(
            f"[inference] Processed {file_path.name}: Triggered {alerts_triggered} alerts."
        )
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        default="/opt/mlflow-artifacts/checkpoints/model_latest_scripted.pt",
        help="Path to TorchScript model checkpoint",
    )
    parser.add_argument(
        "--flows-dir",
        default="/mnt/ramdisk/flows",
        help="Directory where extractor writes CSVs",
    )
    parser.add_argument(
        "--alert-threshold",
        type=float,
        default=0.70,
        help="Confidence threshold to trigger alerts",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--alerts-log",
        default="/mnt/ramdisk/flows/alerts.json",
        help="Output file path for security alerts",
    )
    parser.add_argument(
        "--stats-path",
        default="/root/baseline_stats.json",
        help="Path to baseline stats JSON file for fixed standardization",
    )
    parser.add_argument(
        "--duration",
        type=int,
        default=0,
        help="Optional maximum duration in seconds (0 = infinite)",
    )
    ...
    try:
        while True:
            if args.duration > 0 and (time.time() - start_time) >= args.duration:
                _log.info(
                    f"[inference] Duration limit ({args.duration}s) reached. Exiting gracefully."
                )
                break
