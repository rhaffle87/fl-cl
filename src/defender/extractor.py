...
def calculate_energy_score(logits: np.ndarray, temperature: float = 1.0) -> np.ndarray:
    ...
    if scaled_logits.ndim == 1:
        ...
        lse = max_val.squeeze(-1) + np.log(
            np.sum(np.exp(scaled_logits - max_val), axis=-1)
        )
        return -temperature * lse


def extract_features(
    interface: str, out_dir: str, batch_size: int = 500, streaming_norm: bool = False
):
    """
    ...
    if out_dir.startswith("/mnt/ramdisk") and sys.platform.startswith("linux"):
        if os.path.exists("/mnt/ramdisk") and not os.path.ismount("/mnt/ramdisk"):
            _log.warning(
                "[extractor] WARNING: /mnt/ramdisk is not mounted as tmpfs! Potential disk I/O contention."
            )
    ...
    _log.info(
        f"[extractor] Batch size: {batch_size} flows per file (or max 5 seconds wait)"
    )
...
def main():
    ...
    parser.add_argument(
        "--interface", "-i", default="ens19", help="Capture interface (default: ens19)"
    )
    parser.add_argument(
        "--out-dir",
        "-o",
    ...
    extract_features(
        ...
        args.out_dir,
        args.batch_size,
        streaming_norm=args.streaming_norm,
    )
