...
def profile_model(
    ...
    return {
        ...
        "throughput_fps": float(
            input_tensor.size(0) / (np.mean(latencies_ms) / 1000.0)
        ),
    }
...
def run_profiling(
    ...
    _log.info(
        f"Initiating edge inference profiling for '{model_type}' backbone (Input Dim: {input_dim}, Classes: {num_classes}, Batch Size: {batch_size})..."
    )
    ...
    try:
        quantized_model = torch.quantization.quantize_dynamic(
            baseline_model, {torch.nn.Linear}, dtype=torch.qint8
        )
    ...
    print("=" * 80)
    print(
        f"{'Variant':<20} | {'Mean (ms)':<10} | {'P95 (ms)':<10} | {'P99 (ms)':<10} | {'Throughput (FPS)':<18}"
    )
    print("-" * 80)
    print(
        f"{'Eager FP32':<20} | {eager_stats['mean_ms']:<10.3f} | {eager_stats['p95_ms']:<10.3f} | {eager_stats['p99_ms']:<10.3f} | {eager_stats['throughput_fps']:<18.1f}"
    )
    if scripted_stats:
        print(
            f"{'TorchScript JIT':<20} | {scripted_stats['mean_ms']:<10.3f} | {scripted_stats['p95_ms']:<10.3f} | {scripted_stats['p99_ms']:<10.3f} | {scripted_stats['throughput_fps']:<18.1f}"
        )
    if quant_stats:
        print(
            f"{'Dynamic INT8':<20} | {quant_stats['mean_ms']:<10.3f} | {quant_stats['p95_ms']:<10.3f} | {quant_stats['p99_ms']:<10.3f} | {quant_stats['throughput_fps']:<18.1f}"
        )
    print("=" * 80)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Profile FL-CL Model Inference Latency"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--input-dim", type=int, default=32, help="Feature input dimension"
    )
    parser.add_argument(
        "--num-classes", type=int, default=5, help="Number of output traffic classes"
    )
    parser.add_argument(
        "--batch-size", type=int, default=32, help="Profiling batch size"
    )
