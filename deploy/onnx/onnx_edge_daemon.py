    ...
    def __init__(self, model_path=None, num_threads=4):
        if model_path is None:
            model_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "cyberdefense_cnn.onnx"
            )
        self.model_path = model_path
        if not os.path.exists(model_path):
            raise FileNotFoundError(
                f"ONNX model not found at {model_path}. Run export_champion_onnx.py first."
            )
        ...
        print(
            f"[ONNX Daemon] Loaded edge classifier from {model_path} ({num_threads} CPU threads)"
        )

    def classify_batch(
        self, flow_features: np.ndarray, temperature: float = 1.0
    ) -> tuple:
        """
        ...
        logits = self.session.run([self.output_name], {self.input_name: flow_features})[
            0
        ]
        ...
        energy_scores = -temperature * (
            np.log(np.sum(np.exp(scaled_logits - max_scaled), axis=1))
            + max_scaled.squeeze(1)
        )
...
def run_edge_benchmark(num_flows: int = 10000, batch_size: int = 64):
    ...
    print(
        f"[Benchmark] Evaluating {num_flows} synthetic flows in batches of {batch_size}..."
    )
...
def run_live_daemon(
    flow_dir: str = "/mnt/ramdisk/flows",
    poll_interval: float = 0.5,
    ood_threshold: float = -1.20,
    quarantine_log: str = "/var/log/fl-cl-quarantine.jsonl",
):
    ...
    print(
        f"[ONNX Daemon] Monitoring live flow directory: {flow_dir} (Poll interval: {poll_interval}s, OOD Threshold: {ood_threshold})"
    )
            ...
            for fname in sorted(csv_files):
                ...
                try:
                    ...
                    if data.shape[1] >= 32:
                        ...
                        c_ids, confs, energies, lat = classifier.classify_batch(
                            features
                        )
                        ...
                        for cid, conf, energy in zip(c_ids, confs, energies):
                            ...
                            if energy > ood_threshold:
                                ood_detected += 1
                                print(
                                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [QUARANTINE] Detected Zero-Day OOD Anomaly (Energy: {energy:.2f} > {ood_threshold})"
                                )
                                ...
                                print(
                                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [ALERT] Detected {label_name} (Confidence: {conf*100:.1f}%, Energy: {energy:.2f}) in {lat:.2f}ms"
                                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--benchmark", action="store_true", help="Run synthetic throughput benchmark"
    )
    parser.add_argument(
        "--flow-dir",
        type=str,
        default="/mnt/ramdisk/flows",
        help="Ramdisk flow directory to watch",
    )
    parser.add_argument(
        "--poll-interval", type=float, default=0.5, help="Polling interval in seconds"
    )
    parser.add_argument(
        "--ood-threshold", type=float, default=-1.20, help="OOD energy score threshold"
    )
    parser.add_argument(
        "--quarantine-log",
        type=str,
        default="/var/log/fl-cl-quarantine.jsonl",
        help="Path to quarantine JSONL log",
    )
